...
def show_words_dog():
    lcd_draw_string(draw, 10, 150, "LeftRight|UpDown|GoBack", color=(0, 255, 255), scale=font2, mono_space=False)
    lcd_draw_string(draw, 10, 170, "Square|LiftRotate|Swaying", color=(0, 255, 255), scale=font2, mono_space=False)
        ...
        while not break_luyin:
            ...
            if vol > start_threshold:
                ...
                if sum_vol == 1:
                    ...
                    start_luyin = True
                    start_time = time.time()  # 记录录音开始的时间
            
            if start_luyin:
                elapsed_time = time.time() - start_time  # 计算录音持续时间
                
                # **当音量低于阈值 或 录音时间超过 5 秒，停止录音**
