...
def preload_speak_images1():
    ...
    for i in range(1, pic_num + 1):
        try:
            ...
            speak_images.append(img)
        except Exception as e:
            print(f"Error loading image {i}: {e}")

def preload_speak_images2():
    global speak_images
    speak_images = []
    pic_path = "/home/pi/RaspberryPi-CM4-main/demos/xiaozhi/picture/speak4/"
    expression_name_cs = "speak"
...
def preload_speak_images_display1():
    global ani_num, speak_images, display
    if not speak_images:
        print("Warning: recog_images is empty!")
        return

    ani_num = (ani_num + 1) % len(speak_images)
    display.ShowImage(speak_images[ani_num])

def preload_speak_images_display2():
    global ani_num, speak_images, display
    if not speak_images:
        print("Warning: recog_images is empty!")
        return
...
def speak_anmi():
    ...
    while play_anmi:
        ...
        if elapsed >= frame_duration:
            preload_speak_images_display1()
            preload_speak_images_display2()
...
if __name__ == "__main__":
    init_display() 
    preload_images()
    preload_speak_images1()
    preload_speak_images2()
    init_pyaudio() 
    start_animation()
    run()
